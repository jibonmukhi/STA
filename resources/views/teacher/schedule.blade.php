...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-lg-3">
            ...
            <div class="card mb-3">
                <div class="card-header bg-white">
                    <h6 class="mb-0">{{ trans('calendar.todays_sessions') }}</h6>
                </div>
                <div class="card-body">
                    @if($todaysEvents->count() > 0)
                        @foreach($todaysEvents as $session)
                            @php
                            ...
                            <div class="mb-3 pb-3 border-bottom">
                                <div class="d-flex align-items-start">
                                    <div class="flex-shrink-0 me-2">
                                        <span class="badge bg-{{ $categoryColor }}">{{ \Carbon\Carbon::parse($session->start_time)->format('H:i') }}</span>
                                    </div>
                                    <div class="flex-grow-1">
                                        <h6 class="mb-1 small">{{ $course->title }}</h6>
                                        <p class="mb-0 text-muted small">{{ $session->session_title }}</p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    @else
                        <p class="text-muted small mb-0">{{ trans('calendar.no_sessions_today') }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header bg-white">
                    <h6 class="mb-0">{{ trans('calendar.upcoming_sessions') }}</h6>
                </div>
                <div class="card-body" style="max-height: 400px; overflow-y: auto;">
                    @if($upcomingEvents->count() > 0)
                        @foreach($upcomingEvents as $session)
                            @php
                                $course = $session->course;
                                $categoryColor = dataVaultColor('course_category', $course->category) ?? 'info';
                            @endphp
                            <div class="mb-3 pb-3 border-bottom">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        <h6 class="mb-1 small">{{ $course->title }}</h6>
                                        <p class="mb-1 text-muted small">{{ $session->session_title }}</p>
                                        <p class="mb-0 text-muted small">
                                            <i class="fas fa-calendar me-1"></i>{{ $session->session_date->format('d/m/Y') }}
                                            <i class="fas fa-clock ms-2 me-1"></i>{{ \Carbon\Carbon::parse($session->start_time)->format('H:i') }}
                                        </p>
                                    </div>
                        ...
                        @endforeach
                    @else
                        <p class="text-muted small mb-0">{{ trans('calendar.no_upcoming_sessions') }}</p>
                    @endif
                </div>
            </div>
        </div>
